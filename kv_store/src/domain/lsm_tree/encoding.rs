...
fn read_next_datum<Tr: Read + Seek>(reader: &mut Tr, buffer: &mut Vec<u8>) -> io::Result<usize> {
...
fn skip_next_datum<Tr: Read + Seek>(reader: &mut Tr, buffer: &mut Vec<u8>) -> io::Result<usize> {
    let size = read_size(reader, buffer)?;
